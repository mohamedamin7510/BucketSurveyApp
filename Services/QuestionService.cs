...
public class QuestionService(ApplicationContext context, HybridCache hybridCache, ILogger<QuestionService> logger ) : IQuestionService
{

    private readonly ApplicationContext _Context = context;
    private readonly HybridCache _HybridCache = hybridCache;
    private readonly ILogger<QuestionService> _Logger = logger;
    private const string PerfixKey = "Questions-";
    ...
    public async Task<Result<QuestionResponse>> AddAsync(int PollId, QuestionRequest Request, CancellationToken cancellationToken = default)
    {
        ...
        await _HybridCache.RemoveAsync(PerfixKey + PollId, cancellationToken);
    ...
    public async Task<Result<IEnumerable<QuestionResponse>>> GetAvailiableAsync(int pollid, string? userId, CancellationToken cancellationToken)
    {
        ...
        string Key = PerfixKey + pollid;


      var response  =  await _HybridCache.GetOrCreateAsync<IEnumerable<QuestionResponse>>(
          
          Key , async cancelationtoken => await _Context.Questions.Include(x => x.Answers)
                     .Where(x => x.IsActive && pollid == x.pollId)
                     .Select(x => new QuestionResponse(x.Id, x.Content,x.Answers
                     .Where(x => x.IsActive)
