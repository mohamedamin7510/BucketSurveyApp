...
public class PollService
    ( ApplicationContext context,
      IPollNotificationService pollNotificationService
    ) : IPollService
{
    private readonly ApplicationContext _Context = context;
    private readonly IPollNotificationService _PollNotificationService = pollNotificationService;
    ...
    public async Task<Result> ToggleIsPublishedStatus(int Id, CancellationToken cancellationToken)
    {
        ...
        if (updatedpoll.IsPublished && updatedpoll.StartsAt == DateOnly.FromDateTime(DateTime.UtcNow))
            BackgroundJob.Enqueue(() => _PollNotificationService.SendNewPollNotification(updatedpoll.Id));
